...
%   2012-12 MC added a "sum" to evalstr so it can deal with 3D data...
%
...
%
%  xx = 0:100;
%  yy = gaussian([30 40 5 10],xx)+rand(size(xx));
%  plot(xx,yy,'o');
%  
%  [dist,fitpars]=fitit('gaussian',yy,...
%     [0 0 0 0], [], [100 50 50 30],...
%     [],xx);
%  
%  plot(xx,gaussian(fitpars,xx),'-', xx, yy, 'o');
%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
evalstr = ['sum(sum(sum( (abs(data-' modelstr ').*weights).^2 )))'];
if isnan(evalstr)
